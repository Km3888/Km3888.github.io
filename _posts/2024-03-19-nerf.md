...
In practice, these integrals are approximated using a finite sum. It would suffice decently well to just break up the ray into constant-length intervals for this approximation. It's then pretty trivial to scale up from computing a single pixel to an entire image as it can parallelized across a GPU.
